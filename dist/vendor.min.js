...
// └────────────────────────────────────────────────────────────┘ \\
function init(){Api.initConversation(),Api.setWatsonPayload=appendWastonToWindow}function bindSendButtonEvent(){muted=!1,$("#btn-input").keypress(function(event){13===event.keyCode&&(sendMessage(),event.preventDefault(),$("#btn-input").val(""))}),$("#btn-chat").on("click",function(event){sendMessage(),event.preventDefault(),$("#btn-input").val("")}),$("#SoundButton").on("click",function(){muted?($(this).html('<span class="glyphicon glyphicon-volume-up"></span>sound'),Api.setWatsonPayload({output:{text:["speaker off!"]}}),muted=!0):($(this).html('<span class="glyphicon glyphicon-volume-off"></span>muted'),Api.setWatsonPayload({output:{text:["speaker on!"]}}),muted=!1)})}function sendMessage(newText){var text;text=newText?newText:$("#btn-input").val(),text&&(appendMessageToWindow(text),Api.postConversationMessage(text),$("#btn-input").val(""))}function appendMessageToWindow(text){var msg='<li class="right clearfix"><span class="chat-img pull-right">                    <img src="http://placehold.it/50/FA6F57/fff&amp;text=ME" alt="User Avatar" class="img-circle">                  </span>                  <div class="chat-body clearfix">                    <div class="header">                      <small class=" text-muted"><span class="glyphicon glyphicon-time"></span>'+getNow()+'</small>                      <strong class="pull-right primary-font">Patients</strong>                    </div>                    <p>'+text+"</p>                  </div>                </li>";$("#chat-window").append(msg),$("#panel-body").animate({scrollTop:$("#panel-body").prop("scrollHeight")},"slow")}function appendWastonToWindow(data){var text=extractText(data);muted||TTSModule.playCurrentAudio(text);var msg='<li class="left clearfix">              <span class="chat-img pull-left">              <img src="http://placehold.it/50/55C1E7/fff&amp;text=Dr." alt="User Avatar" class="img-circle">              </span>              <div class="chat-body clearfix">                <div class="header">                  <strong class="primary-font">Doctor A.</strong> <small class="pull-right text-muted">                  <span class="glyphicon glyphicon-time"></span>'+getNow()+"</small>                </div>                <p>"+text+" </p>              </div>            </li>";$("#chat-window").append(msg),$("#panel-body").animate({scrollTop:$("#panel-body").prop("scrollHeight")},"slow")}function extractText(data){var pureText="",outputText=data.output.text;return $.each(outputText,function(indexInArray,value){pureText+=value}),pureText}function getNow(){var dt=new Date,time=dt.getHours()+":"+dt.getMinutes()+":"+dt.getSeconds();return time}/*!
 * Bootstrap v3.3.7 (http://getbootstrap.com)
 * Copyright 2011-2016 Twitter, Inc.
 * Licensed under the MIT license
 */
if(function(glob){var current_event,stop,version="0.4.2",has="hasOwnProperty",separator=/[\.\/]/,comaseparator=/\s*,\s*/,wildcard="*",numsort=function(a,b){return a-b},events={n:{}},firstDefined=function(){for(var i=0,ii=this.length;i<ii;i++)if("undefined"!=typeof this[i])return this[i]},lastDefined=function(){for(var i=this.length;--i;)if("undefined"!=typeof this[i])return this[i]},eve=function(name,scope){name=String(name);var l,oldstop=stop,args=Array.prototype.slice.call(arguments,2),listeners=eve.listeners(name),z=0,indexed=[],queue={},out=[],ce=current_event;out.firstDefined=firstDefined,out.lastDefined=lastDefined,current_event=name,stop=0;for(var i=0,ii=listeners.length;i<ii;i++)"zIndex"in listeners[i]&&(indexed.push(listeners[i].zIndex),listeners[i].zIndex<0&&(queue[listeners[i].zIndex]=listeners[i]));for(indexed.sort(numsort);indexed[z]<0;)if(l=queue[indexed[z++]],out.push(l.apply(scope,args)),stop)return stop=oldstop,out;for(i=0;i<ii;i++)if(l=listeners[i],"zIndex"in l)if(l.zIndex==indexed[z]){if(out.push(l.apply(scope,args)),stop)break;do if(z++,l=queue[indexed[z]],l&&out.push(l.apply(scope,args)),stop)break;while(l)}else queue[l.zIndex]=l;else if(out.push(l.apply(scope,args)),stop)break;return stop=oldstop,current_event=ce,out};eve._events=events,eve.listeners=function(name){var item,items,k,i,ii,j,jj,nes,names=name.split(separator),e=events,es=[e],out=[];for(i=0,ii=names.length;i<ii;i++){for(nes=[],j=0,jj=es.length;j<jj;j++)for(e=es[j].n,items=[e[names[i]],e[wildcard]],k=2;k--;)item=items[k],item&&(nes.push(item),out=out.concat(item.f||[]));es=nes}return out},eve.on=function(name,f){if(name=String(name),"function"!=typeof f)return function(){};for(var names=name.split(comaseparator),i=0,ii=names.length;i<ii;i++)!function(name){for(var exist,names=name.split(separator),e=events,i=0,ii=names.length;i<ii;i++)e=e.n,e=e.hasOwnProperty(names[i])&&e[names[i]]||(e[names[i]]={n:{}});for(e.f=e.f||[],i=0,ii=e.f.length;i<ii;i++)if(e.f[i]==f){exist=!0;break}!exist&&e.f.push(f)}(names[i]);return function(zIndex){+zIndex==+zIndex&&(f.zIndex=+zIndex)}},eve.f=function(event){var attrs=[].slice.call(arguments,1);return function(){eve.apply(null,[event,null].concat(attrs).concat([].slice.call(arguments,0)))}},eve.stop=function(){stop=1},eve.nt=function(subname){return subname?new RegExp("(?:\\.|\\/|^)"+subname+"(?:\\.|\\/|$)").test(current_event):current_event},eve.nts=function(){return current_event.split(separator)},eve.off=eve.unbind=function(name,f){if(!name)return void(eve._events=events={n:{}});var names=name.split(comaseparator);if(names.length>1)for(var i=0,ii=names.length;i<ii;i++)eve.off(names[i],f);else{names=name.split(separator);var e,key,splice,i,ii,j,jj,cur=[events];for(i=0,ii=names.length;i<ii;i++)for(j=0;j<cur.length;j+=splice.length-2){if(splice=[j,1],e=cur[j].n,names[i]!=wildcard)e[names[i]]&&splice.push(e[names[i]]);else for(key in e)e[has](key)&&splice.push(e[key]);cur.splice.apply(cur,splice)}for(i=0,ii=cur.length;i<ii;i++)for(e=cur[i];e.n;){if(f){if(e.f){for(j=0,jj=e.f.length;j<jj;j++)if(e.f[j]==f){e.f.splice(j,1);break}!e.f.length&&delete e.f}for(key in e.n)if(e.n[has](key)&&e.n[key].f){var funcs=e.n[key].f;for(j=0,jj=funcs.length;j<jj;j++)if(funcs[j]==f){funcs.splice(j,1);break}!funcs.length&&delete e.n[key].f}}else{delete e.f;for(key in e.n)e.n[has](key)&&e.n[key].f&&delete e.n[key].f}e=e.n}}},eve.once=function(name,f){var f2=function(){return eve.unbind(name,f2),f.apply(this,arguments)};return eve.on(name,f2)},eve.version=version,eve.toString=function(){return"You are running Eve "+version},"undefined"!=typeof module&&module.exports?module.exports=eve:"function"==typeof define&&define.amd?define("eve",[],function(){return eve}):glob.eve=eve}(this),function(glob,factory){if("function"==typeof define&&define.amd)define(["eve"],function(eve){return factory(glob,eve)});else if("undefined"!=typeof exports){var eve=require("eve");module.exports=factory(glob,eve)}else factory(glob,glob.eve)}(window||this,function(window,eve){
// Copyright (c) 2013 Adobe Systems Incorporated. All rights reserved.
// 
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
 ...
 * limitations under the License.
 */
var STTModule=function(){"use strict";function init(){checkBrowsers()}function checkBrowsers(){navigator.getUserMedia||navigator.webkitGetUserMedia||navigator.mozGetUserMedia||navigator.msGetUserMedia||alert("your browser does not support speech to text service!")}function micON(){recording===!1?0===records?(Api.setWatsonPayload({output:{text:["Accept the microphone prompt in your browser. Watson will listen soon."],ref:"STT"}}),records++):speechToText():(recording=!1,stream.stop())}function speechToText(){mic.setAttribute("class","active-mic"),recording=!0,fetch("/api/speech-to-text/token").then(function(response){return response.text()}).then(function(token){stream=WatsonSpeech.SpeechToText.recognizeMicrophone({token:token,continuous:!1,outputElement:"#btn-input",inactivity_timeout:10,format:!1,keepMicrophone:!0}),stream.promise().then(function(data){if(mic.setAttribute("class","inactive-mic"),recording=!1,0!==data.length){var dialogue=data.pop();""!==dialogue.alternatives[0].transcript&&dialogue["final"]===!0&&sendMessage()}else Api.setWatsonPayload({output:{text:["Microphone input cancelled. Please press the button to speak to Watson again"]}})})["catch"](function(err){"Error: No speech detected for 10s."!==err&&console.log(err),mic.setAttribute("class","inactive-mic"),Api.setWatsonPayload({output:{text:["Watson timed out after a few seconds of inactivity. Please press the button to speak to Watson again."]}})})})["catch"](function(error){console.log(error)})}var stream,mic=document.getElementById("input-mic"),recording=!1,records=0;return{micON:micON,speechToText:speechToText,init:init}}();STTModule.init();var TTSModule=function(){"use strict";function playCurrentAudio(text){fetch("/api/text-to-speech/token").then(function(response){return response.text()}).then(function(token){text?(null===audio||audio.ended||audio.pause(),audio=WatsonSpeech.TextToSpeech.synthesize({text:text,voice:"en-US_MichaelVoice",autoPlay:!0,token:token})):null===audio||audio.ended||audio.pause()})}var audio=null;return{playCurrentAudio:playCurrentAudio}}(),Api=function(){"use strict";function initConversation(){postConversationMessage("")}function postConversationMessage(text){var data={input:{text:text}};context&&(data.context=context),Api.setUserPayload(data);var http=new XMLHttpRequest;http.open("POST",messageEndpoint,!0),http.setRequestHeader("Content-type","application/json; charset=utf-8"),http.onload=function(){if(200===http.status&&http.responseText){var response=JSON.parse(http.responseText);context=response.context,Api.setWatsonPayload(response)}else Api.setWatsonPayload({output:{text:['The service may be down at the moment; please check <a href="https://status.ng.bluemix.net/" target="_blank">here</a> for the current status. <br> If the service is OK, the app may not be configured correctly, please check workspace id and credentials for typos. <br> If the service is running and the app is configured correctly, try refreshing the page and/or trying a different request.']}}),console.error("Server error when trying to reply!")},http.onerror=function(){console.error("Network error trying to send message!")},http.send(JSON.stringify(data))}var userPayload,watsonPayload,context,messageEndpoint="/api/message";return{initConversation:initConversation,postConversationMessage:postConversationMessage,getUserPayload:function(){return userPayload},setUserPayload:function(payload){userPayload=payload},getWatsonPayload:function(){return watsonPayload},setWatsonPayload:function(payload){watsonPayload=payload}}}();!function(){bindSendButtonEvent(),init()}();